...
describe('authService', () => {
  ...
  const mockPrisma = {
    ...
    githubCache: { findUnique: jest.fn(), upsert: jest.fn(), delete: jest.fn() },
    githubRepoData: { findMany: jest.fn(), upsert: jest.fn(), findUnique: jest.fn() },
  } as any;
  ...
  it('register should hash password and return user without password', async () => {
    mockPrisma.user.create.mockResolvedValue({ id: 1, email: 'a@b.com', password: 'hashed-123', usernameGit: 'u' });

    const result = await service.register({ email: 'a@b.com', password: '123', name: '', lastName: '', usernameGit: 'u' } as any);
  ...
  it('login should throw when password invalid', async () => {
    ...
    mockPrisma.user.findUnique.mockResolvedValue({ id: 2, email: 'x@y.com', password: hashedOther });
  ...
  it('login should return token on success', async () => {
    ...
    mockPrisma.user.findUnique.mockResolvedValue({ id: 3, email: 'ok@x.com', password: hashed });
  ...
  it('verifyToken should return decoded payload or null', async () => {
    mockJwt.verify.mockImplementation((t: string) => ({ sub: 1, email: 'a@b' }));
  ...
  it('loginGithub should create user when not exists and return token', async () => {
    ...
    const res = await service.loginGithub({ email: 'g@u.com', username: 'gg', providerId: 123 });
  ...
  it('loginGithub should use existing user and return token', async () => {
    ...
    const res = await service.loginGithub({ email: 'g2@u.com', username: 'gg2', providerId: 124 });
