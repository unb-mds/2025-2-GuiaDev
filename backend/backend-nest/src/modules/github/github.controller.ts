...
export class GithubController {
  ...
  constructor(
    ...
    private readonly docsAnalyzeService: DocsAnalyzerService,
  ) {}
  ...
  async analyzeUserReposByUsername(@Param('username') username: string) {
    try {
      ...
      this.logger.error(
        `Falha ao analisar repositórios para ${username}: ${error.message}`,
        error.stack,
      );
  ...
  async analyzeRepoDocs(
    @Param('owner') owner: string,
    @Param('repo') repo: string,
  ) {
    const [readme, contributing, changelog, conduct, license, docsContent] =
      await Promise.all([
        this.githubService
          .getReadme(owner, repo)
          .catch(() => ({ name: 'README.md', content: null })),
        this.githubService
          .getContributing(owner, repo)
          .catch(() => ({ content: null })),
        this.githubService
          .getChangelog(owner, repo)
          .catch(() => ({ content: null })),
        this.githubService
          .getConductCode(owner, repo)
          .catch(() => ({ content: null })),
        this.githubService
          .getLicenses(owner, repo)
          .catch(() => ({ name: null })),
