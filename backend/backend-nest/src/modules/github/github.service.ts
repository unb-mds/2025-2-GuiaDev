...
export class GithubService {
  ...
  private async httpGet(
    ...
    if (cachedEntry && cachedEntry.etag) {
      if (!finalHeaders['If-None-Match']) {
        ...
        this.logger.debug(
          `[Cache DB] Usando ETag: ${cachedEntry.etag} para ${url}`,
        );
      }
    }
    ...
    try {
      ...
      if (response.status === 304) {
        if (cachedEntry) {
          ...
          this.logger.warn(
            `[Cache DB WARN] 304 recebido sem cache local para ${url}.`,
          );
          throw new Error(
            `304 Not Modified recebido, mas sem cache local para ${url}`,
          );
        }
      }
      ...
      if (cachedEntry) {
        this.logger.warn(
          `[Cache STALE] Request falhou (${status}), retornando cache antigo para ${url}`,
        );
        return {
          ...err.response,
          status: 200,
  ...
  private async _fetchDefaultBranch(
    owner: string,
    repo: string,
  ): Promise<string> {
    try {
      const repoInfoUrl = `https://api.github.com/repos/${owner}/${repo}`;
      const repoRes = await this.httpGet(repoInfoUrl);
      return repoRes.data?.default_branch || 'main';
    } catch (err: any) {
      this.logger.warn(
        `_fetchDefaultBranch: Failed for ${owner}/${repo}: ${err?.message || err}. Assuming 'main'.`,
      );
      return 'main';
    }
  }
  ...
  async getUserRepos(username: string) {
    ...
    while (true) {
      ...
      try {
        ...
        this.logger.warn(`getUserRepos error for ${username}: ${err.message}`);
  ...
  async getCommits(owner: string, repo: string) {
    ...
    try {
      ...
      if (err.status !== 404)
        this.logger.warn(`getCommits ${owner}/${repo}: ${err.message}`);
      return [];
    }
  }
  ...
  async getBranches(owner: string, repo: string) {
    ...
    try {
      const res = await this.httpGet(url);
      return (res.data || []).map((b: any) => b.name).filter(Boolean);
    } catch (err: any) {
      if (err.status !== 404)
        this.logger.warn(`getBranches ${owner}/${repo}: ${err.message}`);
      return [];
    }
  }
  ...
  async getPullRequests(owner: string, repo: string) {
    ...
    try {
      ...
      if (err.status !== 404)
        this.logger.warn(`getPullRequests ${owner}/${repo}: ${err.message}`);
      return [];
    }
  }
  ...
  async getContributors(owner: string, repo: string) {
    ...
    try {
      ...
      if (err.status !== 404)
        this.logger.warn(`getContributors ${owner}/${repo}: ${err.message}`);
      return [];
    }
  }
  ...
  async getIssues(owner: string, repo: string) {
    ...
    try {
      ...
      if (err.status !== 404)
        this.logger.warn(`getIssues ${owner}/${repo}: ${err.message}`);
      return [];
    }
  }
  ...
  async getReleases(owner: string, repo: string) {
    ...
    try {
      ...
      if (err.status !== 404)
        this.logger.warn(`getReleases ${owner}/${repo}: ${err.message}`);
      return [];
    }
  }
  ...
  async getReadme(owner: string, repo: string) {
    try {
      ...
      return {
        ...
        content: this.decodeBase64(response.data.content),
      };
    } catch (err: any) {
      if (err.status !== 404)
        this.logger.warn(`getReadme ${owner}/${repo}: ${err.message}`);
      return { name: 'README.md', path: 'README.md', content: null };
    }
  }
  ...
  async getLicenses(owner: string, repo: string) {
    ...
    try {
      ...
      if (err.status !== 404)
        this.logger.warn(`getLicenses ${owner}/${repo}: ${err.message}`);
      return { name: 'Unlicensed', key: 'unlicensed', fileName: null };
    }
  }
  ...
  async getGitignore(owner: string, repo: string) {
    ...
    try {
      effectiveBranch = await this._fetchDefaultBranch(owner, repo);
    } catch {}
    try {
      const url = `https://api.github.com/repos/${owner}/${repo}/contents/.gitignore`;
      const res = await this.httpGet(url);
      return {
        content: this.decodeBase64(res.data.content),
        sha: res.data.sha,
        path: res.data.path,
      };
    } catch (err: any) {
      if (err.status === 404) {
        try {
          ...
          const treeRes = await this.httpGet(treeUrl);
          const entry = (treeRes.data.tree || []).find(
            (e: any) =>
              e.type === 'blob' &&
              path.basename(e.path).toLowerCase() === '.gitignore',
          );
          if (!entry) return { content: null, sha: null, path: null };
          const blobRes = await this.httpGet(
            `https://api.github.com/repos/${owner}/${repo}/git/blobs/${entry.sha}`,
          );
          return {
            content: this.decodeBase64(blobRes.data?.content),
            sha: entry.sha,
            path: entry.path,
          };
        } catch (treeErr: any) {
          this.logger.warn(
            `getGitignore fallback error for ${owner}/${repo}@${effectiveBranch}: ${treeErr?.message || treeErr}`,
          );
          return { content: null, sha: null, path: null };
        }
      }
      this.logger.warn(
        `getGitignore error for ${owner}/${repo}: ${err.message}`,
      );
      return { content: null, sha: null, path: null };
    }
  }
  ...
  async getChangelog(owner: string, repo: string) {
    ...
    let branch = 'main';
    try {
      branch = await this._fetchDefaultBranch(owner, repo);
    } catch {}
    try {
      for (const cand of candidates) {
        try {
          const url = `https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(cand)}?ref=${encodeURIComponent(branch)}`;
          const res = await this.httpGet(url);
          return {
            content: this.decodeBase64(res.data.content),
            sha: res.data.sha,
      ...
      const entry = (treeRes.data.tree || []).find(
        (e: any) =>
          e.type === 'blob' &&
          candidates.includes(path.basename(e.path).toLowerCase()),
      );
      if (!entry) return { content: null, sha: null, path: null };
      const blobRes = await this.httpGet(
        `https://api.github.com/repos/${owner}/${repo}/git/blobs/${entry.sha}`,
      ...
      if (err.status !== 404)
        this.logger.warn(
          `getChangelog ${owner}/${repo}@${branch}: ${err.message}`,
        );
      return { content: null, sha: null, path: null };
    }
  }
  ...
  async getContributing(owner: string, repo: string) {
    try {
      const branch = await this._fetchDefaultBranch(owner, repo);
      const url = `https://api.github.com/repos/${owner}/${repo}/contents/CONTRIBUTING.md?ref=${encodeURIComponent(branch)}`;
      const res = await this.httpGet(url);
      return {
        content: this.decodeBase64(res.data.content),
        sha: res.data.sha,
      ...
      if (err.status !== 404)
        this.logger.warn(`getContributing ${owner}/${repo}: ${err.message}`);
      return { content: null, sha: null, path: null };
    }
  }
  ...
  async getConductCode(owner: string, repo: string) {
    ...
    try {
      const url = `https://api.github.com/repos/${owner}/${repo}/contents/CODE_OF_CONDUCT.md?ref=${encodeURIComponent(branch)}`;
      const res = await this.httpGet(url);
      return {
        content: this.decodeBase64(res.data.content),
        sha: res.data.sha,
      ...
      if (err.status === 404) {
        try {
          const fallbackUrl = `https://api.github.com/repos/${owner}/${repo}/contents/.github/CODE_OF_CONDUCT.md?ref=${encodeURIComponent(branch)}`;
          const fallbackRes = await this.httpGet(fallbackUrl);
          return {
            content: this.decodeBase64(fallbackRes.data.content),
          ...
          if (fallbackErr.status !== 404)
            this.logger.warn(
              `getConductCode fallback ${owner}/${repo}: ${fallbackErr.message}`,
            );
          return { content: null, sha: null, path: null };
        }
      }
      this.logger.warn(`getConductCode ${owner}/${repo}: ${err.message}`);
      return { content: null, sha: null, path: null };
    }
  }
  ...
  async getDocsContent(owner: string, repo: string) {
    ...
    for (const file of files) {
      try {
        const response = await this.httpGet(file.download_url, {
  ...
  async getRepoTree(owner: string, repo: string) {
    try {
      ...
      const result = tree.map((entry: any) => ({
        path: entry.path,
        type: entry.type,
      }));
      return result;
  ...
  async getGovernance(owner: string, repo: string) {
    try {
      const branch = await this._fetchDefaultBranch(owner, repo);
      const patterns = ['governance.md'];
      const { resultsByPath } = await this.fetchChecklistMdFiles(
        owner,
        repo,
        patterns,
        branch,
      );
      const result = Object.values(resultsByPath).find(
        (r) =>
          r.content && path.basename(r.path).toLowerCase() === 'governance.md',
      );
      return { path: result?.path || null, content: result?.content || null };
    } catch (err: any) {
      if (err.status !== 404)
        this.logger.warn(`getGovernance ${owner}/${repo}: ${err.message}`);
  ...
  async getArchitecture(owner: string, repo: string) {
    try {
      const branch = await this._fetchDefaultBranch(owner, repo);
      const patterns = ['architecture.md', 'arquitetura.md'];
      const { resultsByPath } = await this.fetchChecklistMdFiles(
        owner,
        repo,
        patterns,
      ...
      const result = Object.values(resultsByPath)
        .filter(
          (r) =>
            r.content && patterns.includes(path.basename(r.path).toLowerCase()),
        )
        .sort((a, b) => a.path.length - b.path.length)[0];
      return { path: result?.path || null, content: result?.content || null };
    } catch (err: any) {
      if (err.status !== 404)
        this.logger.warn(`getArchitecture ${owner}/${repo}: ${err.message}`);
      return { path: null, content: null };
    }
  }
  ...
  async getRoadmap(owner: string, repo: string) {
    try {
      ...
      const result = Object.values(resultsByPath).find(
        (r) =>
          r.content && path.basename(r.path).toLowerCase() === 'roadmap.md',
      );
      return { path: result?.path || null, content: result?.content || null };
    } catch (err: any) {
      if (err.status !== 404)
        this.logger.warn(`getRoadmap ${owner}/${repo}: ${err.message}`);
      return { path: null, content: null };
    }
  }

  async fetchChecklistMdFiles(
    owner: string,
    repo: string,
    patterns: string[],
    branch: string,
    ...
    try {
      try {
        ...
        const refRes = await this.httpGet(
          `https://api.github.com/repos/${owner}/${repo}/git/refs/heads/${encodeURIComponent(branch)}`,
        );
        const commitSha = refRes.data.object.sha;
        const commitRes = await this.httpGet(
      ...
      this.logger.warn(
        `fetchChecklistMdFiles: could not fetch tree for ${owner}/${repo}@${branch}: ${err?.message || err}`,
      );
      return {
        patternResults: {},
        resultsByPath: {},
    ...
    const pathMap = new Map(blobs.map((b) => [b.path.toLowerCase(), b]));
    const resolvePattern = (pat: string) => {
      const matches = new Set<string>();
      const ptrim = pat.trim();
      if (ptrim === '/') {
        allPaths.forEach((p) => matches.add(p));
      } else if (ptrim.endsWith('/')) {
        const prefix = ptrim.replace(/^\/+|\/+$/g, '').toLowerCase() + '/';
        for (const p of allPaths)
          if (p.toLowerCase().startsWith(prefix)) matches.add(p);
        ...
        for (const p of allPaths) if (regex.test(p)) matches.add(p);
      } else {
        const norm = ptrim.replace(/^\/+/, '').toLowerCase();
        if (pathMap.has(norm)) {
          const found = blobs.find((b) => b.path.toLowerCase() === norm);
          if (found) matches.add(found.path);
        }
        for (const p of allPaths)
          if (path.basename(p).toLowerCase() === norm) matches.add(p);
      }
      return [...matches].filter((p) => /\.md$/i.test(p));
    };
    const patternMatches: Record<string, string[]> = {};
    const allMatchedPathsSet = new Set<string>();
    for (const patt of patterns) {
      try {
        const m = resolvePattern(patt);
        patternMatches[patt] = m;
        m.forEach((p) => allMatchedPathsSet.add(p));
      } catch {
        patternMatches[patt] = [];
      }
    }
    const allMatchedPaths = [...allMatchedPathsSet];
    const resultsByPath: Record<
      string,
      { path: string; sha?: string; content: string | null; skipped?: string }
    > = {};

    const worker = async (p: string) => {
      const entry = blobs.find((b) => b.path === p);
      if (!entry) {
        resultsByPath[p] = { path: p, content: null, skipped: 'not_found' };
        return;
      }
      if (!/\.md$/i.test(entry.path)) {
        resultsByPath[p] = { path: p, content: null, skipped: 'not_md' };
        return;
      }
      if (entry.size > this.MAX_BYTES) {
        resultsByPath[p] = {
          path: p,
          sha: entry.sha,
          content: null,
      ...
      try {
        ...
        if (this.bufferIsProbablyBinary(buffer)) {
          resultsByPath[p] = {
            path: p,
            sha: entry.sha,
            content: null,
    ...
    > = {};
    for (const patt of patterns) {
      patternResults[patt] = (patternMatches[patt] || []).map(
        (p) =>
          resultsByPath[p] || {
            path: p,
            content: null,
    ...
    const summary = {
      totalPatterns: patterns.length,
      totalMdFilesMatched: allMatchedPaths.length,
      fetched: Object.values(resultsByPath).filter((r) => r.content !== null)
        .length,
      skipped: Object.values(resultsByPath).filter((r) => r.skipped).length,
    };
  ...
  async getFormattedCachedRepos() {
    this.logger.debug(
      `[Cache Read] Buscando todos os repositórios da tabela 'GithubRepoData'...`,
    );
    ...
    this.logger.debug(
      `[Cache Read] Encontrados ${allCache.length} resultados. Retornando dados formatados...`,
    );
  ...
  formatRepoData(reposData: any[]) {
    return reposData.map((repo, index) => {
      ...
      const apiLicenseValid =
        repo.license?.key && repo.license.key !== 'unlicensed';
      ...
      return {
        ...
        prs_count: Array.isArray(repo.pullRequests)
          ? repo.pullRequests.length
          : 0,
        contributors_count: Array.isArray(repo.contributors)
        ...
        pullRequests: Array.isArray(repo.pullRequests)
          ? repo.pullRequests.slice(0, 5)
          : [],
        contributors: Array.isArray(repo.contributors)
          ? repo.contributors.slice(0, 5)
          : [],
  ...
  async checkRepoChanged(
    owner: string,
    repo: string,
    forceUpdate: boolean = false,
  ): Promise<{ changed: boolean; etag: string | null }> {
    ...
    try {
      ...
      if (res.status === 304) {
        ...
        if (cached) {
          ...
          this.logger.warn(
            `[RepoCache] ${owner}/${repo} retornou 304 mas não há cache local.`,
          );
      ...
      this.logger.debug(
        `[RepoCache] ${owner}/${repo} mudou (ou forceUpdate). Novo ETag: ${newEtag}`,
      );
      return { changed: true, etag: newEtag };
    } catch (err: any) {
      this.logger.warn(
        `[RepoCache] Falha ao verificar ${owner}/${repo}: ${err?.message || err}`,
      );

      if (cached) {
        this.logger.warn(
          `[RepoCache] Usando ETag 'stale' para ${owner}/${repo} após falha.`,
        );
        return { changed: false, etag: cached.etag };
      }
  ...
  private async _analyzeSingleRepo(
    repo: {
      owner: { login: string };
      name: string;
    ...
    if (cachedRepoData && !forceUpdate) {
      this.logger.debug(
        `[Analyze] Cache HIT no DB (Lazy) para ${owner}/${name}. Retornando sem verificar GitHub.`,
      );
      return cachedRepoData.data;
    }

    const { changed: repoChanged, etag: repoEtag } =
      await this.checkRepoChanged(owner, name, forceUpdate);

    if (!repoChanged && cachedRepoData && cachedRepoData.etag === repoEtag) {
      this.logger.debug(
        `[Analyze] Cache HIT após verificação (304) para ${owner}/${name}.`,
      );
      return cachedRepoData.data;
    }

    this.logger.debug(
      `[Analyze] Cache MISS, Stale ou ForceUpdate para ${owner}/${name}. Re-analisando...`,
    );
    ...
    try {
      ...
      try {
        ...
        if (chkContent) {
          ...
          for (const tl of tableLines) {
            ...
            if (first && !/^Documento|^Item/i.test(first))
              items.push(first.replace(/`/g, '').trim());
          }
      ...
      const { patternResults, resultsByPath, summary } =
        await this.fetchChecklistMdFiles(owner, name, patterns, defaultBranch);

      const docsContentArray = Object.values(resultsByPath).map((r) => ({
      ...
      const analysisResult = {
        ...
        branches:
          branchesSettled.status === 'fulfilled' ? branchesSettled.value : [],
        pullRequests: prsSettled.status === 'fulfilled' ? prsSettled.value : [],
        contributors:
          contributorsSettled.status === 'fulfilled'
            ? contributorsSettled.value
            : [],

        commits:
          commitsSettled.status === 'fulfilled' ? commitsSettled.value : [],
        issues: issuesSettled.status === 'fulfilled' ? issuesSettled.value : [],
        releases:
          releasesSettled.status === 'fulfilled' ? releasesSettled.value : [],
        readme:
          readmeSettled.status === 'fulfilled'
            ? readmeSettled.value
            : { content: null },
        changelog:
          changelogSettled.status === 'fulfilled'
            ? changelogSettled.value
            : { content: null },
        conductcode:
          conductSettled.status === 'fulfilled'
            ? conductSettled.value
            : { content: null },
        license:
          licenseSettled.status === 'fulfilled'
            ...
            : { name: 'Unlicensed' },
        gitignore:
          gitignoreSettled.status === 'fulfilled'
            ? gitignoreSettled.value
            : { content: null },
        docs:
          docsListSettled.status === 'fulfilled' ? docsListSettled.value : [],
        contributing:
          contributingSettled.status === 'fulfilled'
            ? contributingSettled.value
            : { content: null },
      ...
      try {
        await this.prisma.githubCache.delete({
          where: { url: `analysis_v1/${owner}/${name}` },
        });
      } catch (e) {
        // Ignora
      }
      ...
      this.logger.warn(
        `analyzeUserRepos per-repo error ${owner}/${name}: ${err?.message || err}`,
      );
      return null;
    }
  }
  ...
  async analyzeUserRepos(username: string, forceUpdate: boolean = false) {
    ...
    this.logger.debug(
      `[Analyze] Iniciando análise paralela para ${repos.length} repositórios (ForceUpdate: ${forceUpdate})...`,
    );
    ...
    this.logger.debug(
      `[Analyze] Análise paralela concluída. ${allResults.length} resultados.`,
    );
  ...
  async getCacheByUrl(url: string) {
    ...
    if (!result)
      throw new NotFoundException(`Cache não encontrado para ${url}`);
  ...
  private extractOwner(url: string): string {
    const match =
      url.match(/repos\/([^/]+)/) || url.match(/analysis_v1\/([^/]+)/);
